...
class Quest (JQuest) :
 ...
 def onTalk (self,npc,player):
   ...
   if st :
        ...
        id = st.getState()
        if id == CREATED :
            Ocean = player.getQuestState("q117_OceanOfDistantStar")
   ...
   if st :
        if st.getState() == STARTED and st.getInt("cond") == 1 :
            if st.getRandom(100)<CHANCE :
                st.giveItems(L2jSigmo_FRAGMENT_ID,1)
                st.playSound("ItemSound.quest_itemget")
   return

QUEST       = Quest(650, qn, "A Broken L2jSigmo")
...
STARTED.addQuestDrop(CREWMAN,L2jSigmo_FRAGMENT_ID,1)