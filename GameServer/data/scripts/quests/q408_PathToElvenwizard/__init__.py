# Maked by Mr. Have fun!
# Edited by Bibigon for RT 21.11.07 v 0.3
import sys
...
qn = "q408_PathToElvenwizard"

# Items
ROSELLAS_LETTER = 1218
RED_DOWN = 1219
 ...
 def onEvent (self,event,st) :
    ...
    if event == "1" :
        if player.getClassId().getId() != 0x19 :
          ...
          if st.getInt("cond") != 0 and st.getQuestItemsCount(FERTILITY_PERIDOT) != 0 :
            if st.getQuestItemsCount(MAGICAL_POWERS_RUBY) == 0 :
               if st.getQuestItemsCount(ROSELLAS_LETTER) == 0 :
                  st.giveItems(ROSELLAS_LETTER,1)
          ...
          if st.getInt("cond") != 0 and st.getQuestItemsCount(ROSELLAS_LETTER) != 0 :
            st.takeItems(ROSELLAS_LETTER,st.getQuestItemsCount(ROSELLAS_LETTER))
...
# Rosella
   if npcId == 30414 :
      if st.getInt("cond") == 0 :
         ...
         if st.getQuestItemsCount(ROSELLAS_LETTER) == 0 and st.getQuestItemsCount(APPETIZING_APPLE) == 0 and st.getQuestItemsCount(IMMORTAL_LOVE) == 0 and st.getQuestItemsCount(CHARM_OF_GRAIN) == 0 and st.getQuestItemsCount(SAP_OF_WORLD_TREE) == 0 and st.getQuestItemsCount(LUCKY_POTPOURI) == 0 and st.getQuestItemsCount(FERTILITY_PERIDOT) != 0 :
            if st.getQuestItemsCount(MAGICAL_POWERS_RUBY) == 0 or st.getQuestItemsCount(NOBILITY_AMETHYST) == 0 or st.getQuestItemsCount(PURE_AQUAMARINE) == 0 :
               htmltext = "30414-11.htm"
               if st.getQuestItemsCount(ROSELLAS_LETTER) != 0 :
                  htmltext = "30414-08.htm"
               elif st.getQuestItemsCount(CHARM_OF_GRAIN) != 0 :
                  if st.getQuestItemsCount(RED_DOWN) < 5 :
                     htmltext = "30414-09.htm"
                  else :
                     htmltext = "30414-25.htm"
               elif st.getQuestItemsCount(APPETIZING_APPLE)!=0 :
                  htmltext = "30414-15.htm"
               elif st.getQuestItemsCount(SAP_OF_WORLD_TREE)!=0 and st.getQuestItemsCount(GOLD_LEAVES)<5 :
               ...
               elif st.getQuestItemsCount(IMMORTAL_LOVE) != 0 :
                  htmltext = "30414-19.htm"
               elif st.getQuestItemsCount(LUCKY_POTPOURI) != 0 :
                  if st.getQuestItemsCount(AMETHYST) < 2 :
                     htmltext = "30414-20.htm"
                  else :
                     htmltext = "30414-27.htm"
            else :
               if st.getQuestItemsCount(MAGICAL_POWERS_RUBY) != 0 and st.getQuestItemsCount(NOBILITY_AMETHYST) != 0 and st.getQuestItemsCount(PURE_AQUAMARINE) != 0 :
...
# Sorceress Greenis
   elif npcId == 30157 and st.getInt("cond") != 0 :
      if st.getQuestItemsCount(ROSELLAS_LETTER) != 0 :
...
# Thalia
   elif npcId == 30371 and st.getInt("cond") != 0 :
      if st.getQuestItemsCount(APPETIZING_APPLE)!=0 :
         htmltext = "30371-01.htm"
      elif st.getQuestItemsCount(SAP_OF_WORLD_TREE) != 0 :
...
# Norwind        
   elif npcId == 30423 and st.getInt("cond") != 0 :
      if st.getQuestItemsCount(IMMORTAL_LOVE) != 0 :
...
STARTED.addQuestDrop(30414,ROSELLAS_LETTER,1)
