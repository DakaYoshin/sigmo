...
from com.gameserver.cache import HtmCache
...
class Quest (JQuest) :
 ...
 def onAdvEvent (self,event,npc,player):
   ...
   if event == "30182-05.htm" :
       ...
       htmltext = HtmCache.getInstance().getHtm("data/jscript/quests//" + qn + "/" + npcId + "-14.htm")
       ...
       if selection in guess : #already chose that number!
           htmltext = HtmCache.getInstance().getHtm("data/jscript/quests//" + qn + "/" + npcId + "-" + str(14+2*num_guesses) + ".htm")
       else :
           self.gameStatus[player.getName()][1].append(selection)
           num_guesses += 1
           htmltext = HtmCache.getInstance().getHtm("data/jscript/quests//" + qn + "/" + npcId + "-" + str(11+2*num_guesses) + ".htm")
       htmltext = self.fillBoard(player,htmltext)
   elif event.startswith("select_3-") : #pick #6
       selection = int(event[9])
       guess = self.gameStatus[player.getName()][1]
       if selection in guess : #already chose that number!
           htmltext = HtmCache.getInstance().getHtm("data/jscript/quests//" + qn + "/" + npcId + "-26.htm")
           htmltext = self.fillBoard(player,htmltext)
       else :
           self.gameStatus[player.getName()][1].append(selection)
           ...
           if wins == 3 :
               ...
               st.giveItems(item,1)
               htmltext = HtmCache.getInstance().getHtm("data/jscript/quests//" + qn + "/" + npcId + "-23.htm")
           elif wins == 0 :
               item = self.getReward(eval("Rewards_"+str(bet)+"_Lose"))
               if item == 2437 : st.giveItems(2463,1)
               st.giveItems(item,1)
               htmltext = HtmCache.getInstance().getHtm("data/jscript/quests//" + qn + "/" + npcId + "-25.htm")
           else :
               htmltext = HtmCache.getInstance().getHtm("data/jscript/quests//" + qn + "/" + npcId + "-24.htm")
           guess = self.gameStatus[player.getName()][1]
           for i in range(9) :
              num = self.gameStatus[player.getName()][0][i]
              htmltext = htmltext.replace("<?Cell"+str(i+1)+"?>",str(num))
