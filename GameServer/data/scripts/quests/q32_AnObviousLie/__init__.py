...
class Quest (JQuest) :
 ...
 def onEvent (self,event,st) :
   ...
   if event == "30120-1.htm" :
     ...
     st.set("cond","8")
   elif event in ["cat","racoon","rabbit","romantic","pirate","eye","butterfly","artisan","demonhorns","maskspirit","wizardhat"] :
     if st.getInt("cond") == 8 :
       if st.getQuestItemsCount(THREAD) >= 1000 and st.getQuestItemsCount(SUEDE) >= 500 :
         if event == "cat" :
           ...
           item=YEY
         elif event=="butterfly":
           item=BUTTERFLY
         elif event=="artisan":
           item=ARTISAN
         elif event=="demonhorns":
           item=DEMONHORN
         elif event=="maskspirit":
           item=MASKSPIR
         elif event=="wizardhat":
           item=WIZHAT
