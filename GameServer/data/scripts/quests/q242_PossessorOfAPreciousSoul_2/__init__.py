...
qn = "q242_PossessorOfAPreciousSoul_2"
 ...
 def onEvent (self,event,st) :
   ...
   if event == "30759-5.htm" :
     if cond == 8 :
       ...
       st.playSound("ItemSound.quest_middle")
   if event == "del_pu" :
     if L2World.getInstance().findCharacter(st.getInt("pure_unic")):
       L2World.getInstance().findCharacter(st.getInt("pure_unic")).deleteMe()
   return htmltext
 ...
 def onTalk (self,npc,player):
   ...
   if player.isSubClassActive() :
     ...
     if npcId == PURE_UNICORN and cond == 10 :
       ...
       st.set("pure_unic",str(npc.getObjectId()))
       self.startQuestTimer("del_pu",10000,npc,player)
