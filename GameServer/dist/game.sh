...
do
        ...
        #java -server -Dfile.encoding=UTF- -Xms2g -Xmx3g -Xmn512m -Xss128k -XX:PermSize=128m -XX:MaxPermSize=768m -XX:ParallelGCThreads=2 -XX:+UseConcMarkSweepGC -XX:+UseParNewGC -XX:+CMSIncrementalMode -XX:+CMSIncrementalPacing -XX:CMSIncrementalDutyCycleMin=0 -XX:CMSIncrementalDutyCycle=10 -XX:+CMSParallelRemarkEnabled -XX:SurvivorRatio=8 -XX:TargetSurvivorRatio=90 -XX:MaxTenuringThreshold=31 -XX:+AggressiveOpts -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -Xloggc:gc.txt -cp :lib/*:L2L2jSigmo.jar com.gameserver.GameServer > log/stdout.log 2>&1
		java -Xms1024m -Xmx1024m -cp lib/*:L2L2jSigmo.jar com.gameserver.GameServer > log/stdout.log 2>&1
        err=$?
        sleep 10
done