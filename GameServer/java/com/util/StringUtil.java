 ...
 * 
 * <h1>How to Use It</h1> <h2>concat() or append()</h2> If concatenating strings in single call, use
 * StringUtil.concat(), otherwise use StringUtil.append() and its variants. <h2>Minimum Calls</h2>
 * Bad:
 * 
 * <pre>
 ...
 * memory is used and string concatenation is faster. <h2>Size Hints for Loops</h2> Bad:
 * 
 * <pre>
 * final StringBuilder sbString = new StringBuilder();
 ...
 * 
 * Why?<br/>
 * When using StringUtil.append(), memory is only allocated to fit in the strings in method
 * argument. So on each loop new memory for the string has to be allocated and old string has to be
 * copied to the new string. With size hint, even if the size hint is above the needed memory,
 * memory is saved because new memory has not to be allocated on each cycle. Also it is much faster
 * if no string copy tasks has to be performed. So if concatenating strings in a loop, count
 ...
 * strings, that degrades performance and makes extra memory allocations needed. <h2>Concatenation
 * and Constant Variables</h2> Bad:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Why?</br> In first case, new memory has to be allocated for the second string. In second case
 * already allocated memory is reused, but only in case the new string is not longer than the
 * previously allocated string. Anyway, the second way is better because the string either fits in
 * the memory and some memory is saved, or it does not fit in the memory, and in that case it works
 * as in the first case. <h2>Primitives to Strings</h2> To convert primitives to string, use
 * String.valueOf(). <h2>How much faster is it?</h2> Here are some results of my tests. Count is
 * number of strings concatenated. Don't take the numbers as 100% true as the numbers are affected
 * by other programs running on my computer at the same time. Anyway, from the results it is obvious
 * that using StringBuilder with predefined size is the fastest (and also most memory efficient)
 * solution. It is about 5 times faster when concatenating 7 strings, compared to TextBuilder. Also,
 * with more strings concatenated, the difference between StringBuilder and TextBuilder gets larger.
 * In code, there are many cases, where there are concatenated 50+ strings so the time saving is
 * even greater.
 * 
 * <pre>
 * Count: 2
...
public final class StringUtil
{
	...
	/**
	 * Creates new string builder with size initializated to <code>sizeHint</code>, unless total
	 * length of strings is greater than <code>sizeHint</code>.
	 * 
	 * @param sizeHint
	...
	/**
	 * Pads given string with spaces from right.
	 * 
	 * @param string
	 *            string
	 * @param length
	...
	/**
	 * Pads given string with spaces from left.
	 * 
	 * @param string
	 *            string
	 * @param length
