...
public class AdminVip implements IAdminCommandHandler {
		...
		if (days > 0) {
			...
			try {
				...
				PreparedStatement statement = connection
						.prepareStatement("UPDATE characters SET vip = 1, vip_end = ? WHERE obj_id = ?");
				...
				GmListTable.broadcastMessageToGMs("Admin " + activeChar.getName() + " set vip stat for player "
						+ _playername + " for " + _time + " day(s)");
				_player.sendChatMessage(0, 0, "SYS", "You are now an Vip, Congratulations!");
				_player.broadcastUserInfo();
			} catch (Exception e) {
				_log.warn("could not set vip stats of char: " + e.getMessage(), e);
			} finally {
	...
	public void removeVip(L2PcInstance activeChar, L2PcInstance _player, String _playername) {
		...
		try {
			...
			PreparedStatement statement = connection
					.prepareStatement("UPDATE characters SET vip = 0, vip_end = 0 WHERE obj_id = ?");
			...
			GmListTable.broadcastMessageToGMs(
					"Admin " + activeChar.getName() + " remove vip stat of player " + _playername);
			_player.sendChatMessage(0, 0, "SYS", "Admin removed your Vip Stats!");
			_player.broadcastUserInfo();
		} catch (Exception e) {
			_log.warn("could not remove vip stats of char: " + e.getMessage(), e);
		} finally {
			CloseUtil.close(connection);
		}
	}
