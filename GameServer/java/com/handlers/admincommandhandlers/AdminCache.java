	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar) {
		...
		switch (commandEnum) {
			case admin_cache_htm_reload:
				...
				activeChar.sendChatMessage(0, 0, "SYS", "Cache[HTML]: " + HtmCache.getInstance().getMemoryUsage()
						+ " MB on " + HtmCache.getInstance().getLoadedFiles() + " file(s) loaded.");
				break;
			...
			case admin_cache_reload_path:
				try {
					...
					activeChar.sendChatMessage(0, 0, "SYS", "Cache[HTML]: " + HtmCache.getInstance().getMemoryUsage()
							+ " MB in " + HtmCache.getInstance().getLoadedFiles() + " file(s) loaded.");
				} catch (Exception e) {
			...
			case admin_cache_crest_rebuild:
				...
				activeChar.sendChatMessage(0, 0, "SYS",
						"Cache[Crest]: " + String.format("%.3f", CrestCache.getInstance().getMemoryUsage())
