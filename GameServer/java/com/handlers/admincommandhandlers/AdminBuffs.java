...
public class AdminBuffs implements IAdminCommandHandler {
		...
		if (command.startsWith("admin_getbuffs")) {
			...
			if (st.hasMoreTokens()) {
				...
				if (player != null) {
					...
					activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append("The player ").append(playername)
							.append(" is not online.").toString());
					return false;
				}
			...
			try {
				...
				activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append("Failed removing effect: ")
			...
			try {
				...
				activeChar.sendChatMessage(0, 0, "SYS",
						new StringBuilder().append("All effects canceled within raidus ").append(radius).toString());
				return true;
			} catch (Exception e) {
		...
		if ((target != null) && (skillId > 0)) {
			...
			for (L2Effect e : effects) {
				if ((e != null) && (e.getSkill().getId() == skillId)) {
					e.exit();
					activeChar.sendChatMessage(0, 0, "SYS",
							new StringBuilder().append("Removed ").append(e.getSkill().getName()).append(" level ")
									.append(e.getSkill().getLevel()).append(" from ").append(objId).toString());
				}
			}
	...
	private void removeAllBuffs(L2PcInstance activeChar, int objId) {
		...
		if (target != null) {
			...
			activeChar.sendChatMessage(0, 0, "SYS",
					new StringBuilder().append("Removed all effects from ").append(objId).toString());
			showBuffs(activeChar, target, 1);
		} else {
			activeChar.sendChatMessage(0, 0, "SYS",
					new StringBuilder().append("The player ").append(objId).append(" is not online.").toString());
		}
	}
