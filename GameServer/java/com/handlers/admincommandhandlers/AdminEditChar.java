...
import com.gameserver.network.serverpackets.SetSummonRemainTime;
...
public class AdminEditChar implements IAdminCommandHandler {
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar) {
		...
		if (command.equals("admin_current_player")) {
			...
			try {
				...
				activeChar.sendPacket(new SystemMessage(SystemMessageId.GM_S1)
						.addString("You didnt enter a character name to find."));
			...
			try {
				...
				player.sendPacket(
						new SystemMessage(SystemMessageId.GM_S1).addString("You have been recommended by a GM"));
				player.broadcastUserInfo();
			...
			if (player.getClan() == null) {
				...
				player.sendChatMessage(0, 0, "SYS",
						"A GM Has reset your clan wait time, You may now join another clan.");
				activeChar.sendChatMessage(0, 0, "SYS",
						"You have reset " + player.getName() + "'s wait time to join another clan.");
			} else {
				activeChar.sendChatMessage(0, 0, "SYS", "Sorry, but " + player.getName()
						+ " must not be in a clan. Player must leave clan before the wait limit can be reset.");
			}
		}
	...
	private void listCharacters(L2PcInstance activeChar, int page) {
		...
		for (int x = 0; x < MaxPages; x++) {
			int pagenr = x + 1;
			replyMSG.append(
					"<center><a action=\"bypass -h admin_show_characters " + x + "\">Page " + pagenr + "</a></center>");
		}
		...
		for (int i = CharactersStart; i < CharactersEnd; i++) {
			replyMSG.append("<tr><td width=80><a action=\"bypass -h admin_character_info " + players[i].getName()
					+ "\">" + players[i].getName() + "</a></td><td width=110>" + players[i].getTemplate().className
					+ "</td><td width=40>" + players[i].getLevel() + "</td></tr>");
		}
	...
	private void setTargetKarma(L2PcInstance activeChar, int newKarma) {
		...
		if (newKarma >= 0) {
			...
			player.sendPacket(new SystemMessage(SystemMessageId.GM_S1)
					.addString("Admin has changed your karma from " + oldKarma + " to " + newKarma + "."));

			if (player != activeChar) {
				activeChar.sendChatMessage(0, 0, "SYS", "Successfully Changed karma for " + player.getName() + " from ("
	...
	private void adminModifyCharacter(L2PcInstance activeChar, String modifications) {
		...
		player.sendChatMessage(0, 0, "SYS", "Admin has changed your stats." + "  HP: " + hpval + "  MP: " + mpval
				+ "  CP: " + cpval + "  PvP Flag: " + pvpflagval + " PvP/PK " + pvpkillsval + "/" + pkkillsval);
		...
		player.sendChatMessage(0, 0, "SYS", "Changed stats of " + player.getName() + "." + "  HP: " + hpval + "  MP: "
				+ mpval + "  CP: " + cpval + "  PvP: " + pvpflagval + " / " + pvpkillsval);
	...
	private void findCharacter(L2PcInstance activeChar, String CharacterToFind) {
		...
		for (L2PcInstance player : players) {
			...
			if (name.toLowerCase().contains(CharacterToFind.toLowerCase())) {
				...
				replyMSG.append("<tr><td width=80><a action=\"bypass -h admin_character_list " + name + "\">" + name
						+ "</a></td><td width=110>" + player.getTemplate().className + "</td><td width=40>"
	...
	private void findCharactersPerIp(L2PcInstance activeChar, String IpAdress) throws IllegalArgumentException {
		...
		if (CharactersFound == 0) {
			...
			replyMSG.append(
					"s.<br>In order to avoid you a client crash I won't <br1>display results beyond the 20th character.");
		} else if (CharactersFound == 1) {
			replyMSG.append(".");
		} else {
			replyMSG.append("s.");
