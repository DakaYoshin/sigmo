...
import com.gameserver.model.L2DropCategory;
import com.gameserver.model.L2DropData;
...
public class AdminEditNpc implements IAdminCommandHandler {
		...
		if (command.startsWith("admin_showShop ")) {
			...
			try {
				...
				if (st.countTokens() == 3) {
					...
					activeChar.sendChatMessage(0, 0, "SYS",
							"Usage: //edit_drop <npc_id> <item_id> <category> [<min> <max> <chance>]");
			} catch (StringIndexOutOfBoundsException e) {
				activeChar.sendChatMessage(0, 0, "SYS",
						"Usage: //edit_drop <npc_id> <item_id> <category> [<min> <max> <chance>]");
			}
			...
			try {
				...
				if (st.countTokens() == 1) {
					...
					activeChar.sendChatMessage(0, 0, "SYS",
							"Usage: //add_drop <npc_id> [<item_id> <category> <min> <max> <chance>]");
			} catch (StringIndexOutOfBoundsException e) {
				activeChar.sendChatMessage(0, 0, "SYS",
						"Usage: //add_drop <npc_id> [<item_id> <category> <min> <max> <chance>]");
			}
	...
	private void editShopItem(L2PcInstance activeChar, String[] args) {
		...
		if (args.length > 3) {
			...
			activeChar.sendChatMessage(0, 0, "SYS",
					"Updated price for " + item.getName() + " in Trade List " + tradeListID);
		...
		replyMSG.append("<tr><td>Price</td><td><edit var=\"price\" width=80></td><td>"
				+ tradeList.getPriceForItemId(itemID) + "</td></tr>");
		replyMSG.append("</table>");
		replyMSG.append("<center><br><br><br>");
		replyMSG.append("<button value=\"Save\" action=\"bypass -h admin_editShopItem " + tradeListID + " " + itemID
				+ " $price\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
		replyMSG.append("<br><button value=\"Back\" action=\"bypass -h admin_showShopList " + tradeListID
				+ " 1\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
	...
	private void delShopItem(L2PcInstance activeChar, String[] args) {
		...
		if (args.length > 3) {
			...
			activeChar.sendChatMessage(0, 0, "SYS", "Deleted " + ItemTable.getInstance().getTemplate(itemID).getName()
					+ " from Trade List " + tradeListID);
			showShopList(activeChar, tradeListID, 1);
			return;
		}
	...
	private void addShopItem(L2PcInstance activeChar, String[] args) {
		...
		if (args.length > 3) {
			...
			activeChar.sendChatMessage(0, 0, "SYS",
					"Added " + newItem.getItem().getName() + " to Trade List " + tradeList.getListId());
			showShopList(activeChar, tradeListID, 1);
			return;
		}
		...
		replyMSG.append("<button value=\"Save\" action=\"bypass -h admin_addShopItem " + tradeListID
				+ " $itemID $price\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
		replyMSG.append("<br><button value=\"Back\" action=\"bypass -h admin_showShopList " + tradeListID
				+ " 1\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
		replyMSG.append("</center>");
		replyMSG.append("</body></html>");
	...
	private TextBuilder itemListHtml(L2TradeList tradeList, int page) {
		...
		for (L2ItemInstance item : tradeList.getItems(start, end + 1)) {
			replyMSG.append("<tr><td><a action=\"bypass -h admin_editShopItem " + tradeList.getListId() + " "
					+ item.getItemId() + "\">" + item.getItem().getName() + "</a></td>");
			replyMSG.append("<td>" + item.getPriceToSell() + "</td>");
			replyMSG.append("<td><button value=\"Del\" action=\"bypass -h admin_delShopItem " + tradeList.getListId()
					+ " " + item.getItemId() + "\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
			replyMSG.append("</tr>");
		}
		replyMSG.append("<tr>");
		int min = 1;
		...
		if (page > 1) {
			replyMSG.append("<td><button value=\"Page" + (page - 1) + "\" action=\"bypass -h admin_showShopList "
					+ tradeList.getListId() + " " + (page - 1)
		...
		replyMSG.append(
				"<button value=\"Close\" action=\"bypass -h admin_close_window\" width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
	...
	private void showShop(L2PcInstance activeChar, int merchantID) {
		...
		for (L2TradeList tradeList : tradeLists) {
			if (tradeList != null)
				replyMSG.append("<tr><td><a action=\"bypass -h admin_showShopList " + tradeList.getListId()
						+ " 1\">Trade List " + tradeList.getListId() + "</a></td></tr>");
		}
	...
	private void storeTradeList(int itemID, int price, int tradeListID, int order) {
		...
		try {
			...
			PreparedStatement stmt = con
					.prepareStatement("INSERT INTO merchant_buylists (`item_id`,`price`,`shop_id`,`order`) values ("
	...
	private void updateTradeList(int itemID, int price, int tradeListID, int order) {
		...
		try {
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement stmt = con.prepareStatement("UPDATE merchant_buylists SET `price`='" + price
	...
	private void deleteTradeList(int tradeListID, int order) {
		...
		try {
			...
			PreparedStatement stmt = con.prepareStatement(
					"DELETE FROM merchant_buylists WHERE `shop_id`='" + tradeListID + "' AND `order`='" + order + "'");
			stmt.execute();
			stmt.close();
		} catch (SQLException esql) {
			esql.printStackTrace();
	...
	private int findOrderTradeList(int itemID, int price, int tradeListID) {
		...
		try {
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM merchant_buylists WHERE `shop_id`='"
	...
	private void saveNpcProperty(L2PcInstance activeChar, String command) {
		...
		try {
			...
			if (statToSet.equals("templateId"))
				...
				Class.forName("com.gameserver.model.actor.instance." + value + "Instance");
	...
	private void showNpcDropList(L2PcInstance activeChar, int npcId, int page) {
		...
		replyMSG.append(
				")</font></center><br>Drop type legend: <font color=\"3BB9FF\">Drop</font> | <font color=\"00ff00\">Sweep</font> | <font color=\"C12869\">Quest</font><br1><table width=\"100%\" border=0><tr><td width=35>cat.</td><td width=240>item</td><td width=25>del</td></tr>");
		...
		replyMSG.append(npcId);
		replyMSG.append(
				"\" width=70 height=21 back=\"L2UI.DefaultButton_click\" fore=\"L2UI.DefaultButton\"><button value=\"Close\" action=\"bypass -h admin_close_window\" width=70 height=21 back=\"L2UI.DefaultButton_click\" fore=\"L2UI.DefaultButton\"></center></body></html>");
	...
	private void showEditDropData(L2PcInstance activeChar, int npcId, int itemId, int category) {
		...
		if (dropData != null) {
			...
			replyMSG.append(
					" $min $max $chance\" width=70 height=21 back=\"L2UI.DefaultButton_click\" fore=\"L2UI.DefaultButton\">");
		} else {
		...
		replyMSG.append(npcId);
		replyMSG.append(
				"\" width=100 height=20 back=\"L2UI.DefaultButton_click\" fore=\"L2UI.DefaultButton\"></center>");
		replyMSG.append("</body></html>");
	...
	private void showAddDropData(L2PcInstance activeChar, L2NpcTemplate npcData) {
		...
		replyMSG.append("<button value=\"SAVE\" action=\"bypass -h admin_add_drop " + npcData.npcId
				+ " $itemId $category $min $max $chance\"  width=70 height=21 back=\"L2UI.DefaultButton_click\" fore=\"L2UI.DefaultButton\">");
		replyMSG.append("<button value=\"Back\" action=\"bypass -h admin_show_droplist " + npcData.npcId
	...
	private void updateDropData(L2PcInstance activeChar, int npcId, int itemId, int min, int max, int category,
		...
		try {
			...
			PreparedStatement statement = con.prepareStatement(
					"UPDATE droplist SET min=?, max=?, chance=? WHERE mobId=? AND itemId=? AND category=?");
			...
			PreparedStatement statement2 = con
					.prepareStatement("SELECT mobId FROM droplist WHERE mobId=? AND itemId=? AND category=?");
	...
	private void addDropData(L2PcInstance activeChar, int npcId, int itemId, int min, int max, int category,
			int chance) {
		java.sql.Connection con = null;
	...
	private void deleteDropData(L2PcInstance activeChar, int npcId, int itemId, int category) {
		...
		try {
			...
			if (npcId > 0) {
				PreparedStatement statement2 = con
						.prepareStatement("DELETE FROM droplist WHERE mobId=? AND itemId=? AND category=?");
				statement2.setInt(1, npcId);
				statement2.setInt(2, itemId);
				statement2.setInt(3, category);
				statement2.execute();
				...
				NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
				TextBuilder replyMSG = new TextBuilder("<html><title>Delete drop data(" + npcId + ", " + itemId + ", "
						+ category + ")complete</title>");
				replyMSG.append("<body>");
				replyMSG.append("<center><button value=\"DropList\" action=\"bypass -h admin_show_droplist " + npcId
						+ "\" width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></center>");
				replyMSG.append("</body></html>");
		...
		try {
			...
			PreparedStatement statement = con.prepareStatement("SELECT "
					+ L2DatabaseFactory.getInstance()
							.safetyString(new String[] { "mobId", "itemId", "min", "max", "category", "chance" })
	...
	private void showNpcSkillList(L2PcInstance activeChar, int npcId, int page) {
		...
		StringBuffer replyMSG = new StringBuffer(
				"<html><title>Show NPC Skill List</title><body><center><font color=\"LEVEL\">");
	...
	private void showNpcSkillEdit(L2PcInstance activeChar, int npcId, int skillId) {
		try {
			...
			if (npcSkill != null) {
				...
				replyMSG.append(skillId);
				replyMSG.append(
						" $level\" width=70 height=21 back=\"L2UI.DefaultButton_click\" fore=\"L2UI.DefaultButton\"><br1><button value=\"Back\" action=\"bypass -h admin_show_skilllist_npc ");
				replyMSG.append(npcId);
				replyMSG.append(
	...
	private void updateNpcSkillData(L2PcInstance activeChar, int npcId, int skillId, int level) {
		...
		try {
			...
			if (skillData == null) {
				activeChar.sendChatMessage(0, 0, "SYS",
						"Could not update npc skill: not existing skill id with that level!");
				showNpcSkillEdit(activeChar, npcId, skillId);
				return;
			}
			...
			if (skillData.getLevel() != level) {
				activeChar.sendChatMessage(0, 0, "SYS",
						"Skill id with requested level doesn't exist! Skill level not changed.");
				showNpcSkillEdit(activeChar, npcId, skillId);
				return;
			}
			...
			PreparedStatement statement = con
					.prepareStatement("UPDATE `npcskills` SET `level`=? WHERE `npcid`=? AND `skillid`=?");
	...
	private void showNpcSkillAdd(L2PcInstance activeChar, int npcId) {
		...
		replyMSG.append(npcData.npcId);
		replyMSG.append(
				" $skillId $level\"  width=70 height=21 back=\"L2UI.DefaultButton_click\" fore=\"L2UI.DefaultButton\"><br1><button value=\"Back\" action=\"bypass -h admin_show_skilllist_npc ");
		replyMSG.append(npcData.npcId);
		replyMSG.append(
				"\" width=70 height=21 back=\"L2UI.DefaultButton_click\" fore=\"L2UI.DefaultButton\"></center></body></html>");
	...
	private void addNpcSkillData(L2PcInstance activeChar, int npcId, int skillId, int level) {
		...
		try {
			...
			if (skillData == null) {
				activeChar.sendChatMessage(0, 0, "SYS",
						"Could not add npc skill: not existing skill id with that level!");
				showNpcSkillAdd(activeChar, npcId);
				return;
			}
			...
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con
					.prepareStatement("INSERT INTO `npcskills`(`npcid`, `skillid`, `level`) VALUES(?,?,?)");
			statement.setInt(1, npcId);
	...
	private void deleteNpcSkillData(L2PcInstance activeChar, int npcId, int skillId) {
		...
		try {
			...
			if (npcId > 0) {
				PreparedStatement statement = con
						.prepareStatement("DELETE FROM `npcskills` WHERE `npcid`=? AND `skillid`=?");
				statement.setInt(1, npcId);
				statement.setInt(2, skillId);
				statement.execute();
				statement.close();
	...
	private void reloadNpcSkillList(int npcId) {
		...
		try {
			...
			// without race
			PreparedStatement statement = con.prepareStatement(
					"SELECT `skillid`, `level` FROM `npcskills` WHERE `npcid`=? AND `skillid` <> 4416");
			statement.setInt(1, npcId);
