...
public class AdminCreateItem implements IAdminCommandHandler {
		...
		if (command.equals("admin_itemcreate")) {
			...
			Collection<L2PcInstance> pls = L2World.getInstance().getAllPlayers();
			{
				for (L2PcInstance onlinePlayer : pls) {
					if (activeChar != onlinePlayer && onlinePlayer.isOnline() == 1
							&& (onlinePlayer.getClient() != null)) {
						onlinePlayer.getInventory().addItem("Admin", idval, numval, onlinePlayer, activeChar);
						onlinePlayer.sendChatMessage(0, 0, "SYS",
	...
	private void createItem(L2PcInstance activeChar, L2PcInstance target, int id, int num) {
		...
		if (activeChar != target)
			target.sendChatMessage(0, 0, "SYS",
					"Admin spawned " + num + " " + template.getName() + " in your inventory!");
		activeChar.sendChatMessage(0, 0, "SYS", "You have spawned " + num + " " + template.getName() + "(" + id
				+ ") in " + target.getName() + " inventory.");
	}
