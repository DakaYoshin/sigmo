...
public class AdminSpawn implements IAdminCommandHandler {
	...
	public static final Logger _log = LoggerFactory.getLogger(AdminSpawn.class);
	...
	private void spawnMonster(L2PcInstance activeChar, String monsterId, int respawnTime, int mobCount,
		...
		try {
			...
			if (RaidBossSpawnManager.getInstance().isDefined(spawn.getNpcid())
					|| GrandBossManager.getInstance().getStatsSet(spawn.getNpcid()) != null) {
				...
				permanent = false;
				spawn.set_customBossInstance(true); // for raids, this value is used in order to segnalate to not save
													// respawn time - status for custom instance

			}

			if (RaidBossSpawnManager.getInstance().getValidTemplate(spawn.getNpcid()) != null) {
				RaidBossSpawnManager.getInstance().addNewSpawn(spawn, 0, template1.getStatsSet().getDouble("baseHpMax"),
						template1.getStatsSet().getDouble("baseMpMax"), permanent);
			} else {
	...
	private void showMonsters(L2PcInstance activeChar, int level, int from) {
		...
		tb.append("<html><title>Spawn Monster:</title><body><p> Level " + level + ":<br>Total Npc's : " + mobs.length
				+ "<br>");
		String end1 = "<br><center><button value=\"Next\" action=\"bypass -h admin_spawn_index " + level
		...
		for (int i = from; i < mobs.length; i++) {
			String txt = "<a action=\"bypass -h admin_spawn_monster " + mobs[i].npcId + "\">" + mobs[i].name
					+ "</a><br1>";
	...
	private void showNpcs(L2PcInstance activeChar, String starting, int from) {
		...
		tb.append("<html><title>Spawn Monster:</title><body><p> There are " + mobs.length
				+ " Npcs whose name starts with " + starting + ":<br>");
		String end1 = "<br><center><button value=\"Next\" action=\"bypass -h admin_npc_index " + starting
				+ " $from$\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></center></body></html>";
