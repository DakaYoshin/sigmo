 ...
 * - ban_acc <account_name> = changes account access level to -100 and logs him
 * off. If no account is specified target's account is used.
 * - ban_char <char_name> = changes a characters access level to -100 and logs
 * him off. If no character is specified target is used.
 * - ban_chat <char_name> <duration> = chat bans a character for the specified
 * duration. If no name is specified the target is chat banned indefinitely.
 ...
 * - unban_chat <char_name> = lifts chat ban from specified player. If no player
 * name is specified current target is used.
 * - jail charname [penalty_time] = jails character. Time specified in minutes.
 * For ever if no time is specified.
...
public class AdminBan implements IAdminCommandHandler {
        ...
        if (st.hasMoreTokens()) {
            ...
            if (st.hasMoreTokens()) {
                try {
                    ...
                    activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append("Invalid number format used: ")
                            .append(nfe).append(".").toString());
                    return false;
                }
            }
        } else {
        ...
        if (command.startsWith("admin_ban ") || command.equalsIgnoreCase("admin_ban")) {
            ...
            if (targetPlayer == null && player.equals("")) {
                ...
                activeChar.sendMessage(
                        new StringBuilder().append("Ban request sent for account ").append(player).toString());
                auditAction(command, activeChar, player);
            } else {
                targetPlayer.setPunishLevel(L2PcInstance.PunishLevel.ACC, 0);
                activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append("Account ")
                        .append(targetPlayer.getAccountName()).append(" banned.").toString());
                auditAction(command, activeChar, targetPlayer.getAccountName());
            }
        } else if (command.startsWith("admin_ban_char")) {
            if (targetPlayer == null && player.equals("")) {
                activeChar.sendChatMessage(0, 0, "SYS",
                        "Usage: //ban_char <char_name> (if none, target char is banned).");
                return false;
            } else {
            ...
            if (targetPlayer != null) {
                if (targetPlayer.getPunishLevel().value() > 0) {
                    activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append(targetPlayer.getName())
                            .append(" is already jailed or banned.").toString());
                    return false;
                }
                String banLengthStr = "";
                ...
                activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append(targetPlayer.getName())
                        .append(" is now chat banned").append(banLengthStr).append(".").toString());
            ...
            if (targetPlayer != null) {
                if (targetPlayer.isChatBanned()) {
                    ...
                    activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append(targetPlayer.getName())
                            .append("'s chat ban has now been lifted.").toString());
                    auditAction(command, activeChar, targetPlayer.getName());
                } else {
                    activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append(targetPlayer.getName())
                            .append(" is not currently chat banned.").toString());
                }
            } else {
            ...
            activeChar.sendChatMessage(0, 0, "SYS",
                    "Available unban commands: //unban_acc, //unban_char, //unban_chat .");
            return false;
        } else if (command.startsWith("admin_unban_acc")) {

            if (targetPlayer != null) {
                activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append(targetPlayer.getName())
                        .append(" is currently online so must not be banned.").toString());
                return false;
            } else if (!player.equals("")) {
                LoginServerThread.getInstance().sendAccessLevel(player, 0);
                activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append("Unban request sent for account ")
                        .append(player).append(".").toString());
                auditAction(command, activeChar, player);
            } else {
            ...
            if (targetPlayer != null) {
                ...
                activeChar.sendChatMessage(0, 0, "SYS",
                        new StringBuilder().append("Character ").append(targetPlayer.getName()).append(" jailed for ")
                                .append(duration > 0
                                        ? new StringBuilder().append(duration).append(" minutes.").toString()
                                        : "ever!")
                                .toString());
                auditAction(command, activeChar, targetPlayer.getName());
            } else {
            ...
            if (targetPlayer == null && player.equals("")) {
                activeChar.sendChatMessage(0, 0, "SYS",
                        "Usage: //unjail <charname> (If no name is given target is used)");
                return false;
            } else if (targetPlayer != null) {
                targetPlayer.setPunishLevel(L2PcInstance.PunishLevel.NONE, 0);
                activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append("Character ")
                        .append(targetPlayer.getName()).append(" removed from jail.").toString());
                auditAction(command, activeChar, targetPlayer.getName());
            } else {
    ...
    private void auditAction(String fullCommand, L2PcInstance activeChar, String target) {
        ...
        GMAudit.auditGMAction(activeChar.getName() + " [" + activeChar.getObjectId() + "]", command[0],
                (target.equals("") ? "no-target" : target), (command.length > 2 ? command[2] : ""));
    }
    ...
    private void banChatOfflinePlayer(L2PcInstance activeChar, String name, int delay, boolean ban) {
        ...
        try {
            ...
            if (count == 0)
                ...
                activeChar.sendChatMessage(0, 0, "SYS",
                        new StringBuilder().append("Character ").append(name).append(" chat-banned for ").append(
                                delay > 0 ? new StringBuilder().append(delay).append(" minutes.").toString() : "ever!")
                                .toString());
            else
    ...
    private void jailOfflinePlayer(L2PcInstance activeChar, String name, int delay) {
        ...
        try {
            ...
            if (count == 0)
                ...
                activeChar.sendChatMessage(0, 0, "SYS",
                        new StringBuilder().append("Character ").append(name).append(" jailed for ").append(
                                delay > 0 ? new StringBuilder().append(delay).append(" minutes.").toString() : "ever!")
                                .toString());
        } catch (SQLException se) {
    ...
    private void unjailOfflinePlayer(L2PcInstance activeChar, String name) {
        ...
        try {
            ...
            if (count == 0)
                ...
                activeChar.sendChatMessage(0, 0, "SYS",
                        new StringBuilder().append("Character ").append(name).append(" removed from jail.").toString());
        } catch (SQLException se) {
            activeChar.sendChatMessage(0, 0, "SYS", "SQLException while jailing player.");
        } finally {
            try {
    ...
    private boolean changeCharAccessLevel(L2PcInstance targetPlayer, String player, L2PcInstance activeChar, int lvl) {
        if (targetPlayer != null) {
            ...
            activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append("The character ")
                    .append(targetPlayer.getName()).append(" has now been banned.").toString());
        } else {
            Connection con = null;
            try {
                con = L2DatabaseFactory.getInstance().getConnection();
                ...
                if (count == 0) {
                    ...
                    activeChar.sendChatMessage(0, 0, "SYS", new StringBuilder().append(player)
                            .append(" now has an access level of ").append(lvl).toString());
            } catch (SQLException se) {
