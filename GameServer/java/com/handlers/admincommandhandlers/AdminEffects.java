...
import com.gameserver.network.serverpackets.ExRedSky;
...
import com.gameserver.network.serverpackets.SunRise;
import com.gameserver.network.serverpackets.SunSet;
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar) {
		...
		if (command.startsWith("admin_invis")) {
			...
			try {
				...
				Earthquake eq = new Earthquake(activeChar.getX(), activeChar.getY(), activeChar.getZ(), intensity,
						duration);
						...
						if (player != null) {
							if (performSocial(social, player, activeChar)) {
								activeChar.sendChatMessage(0, 0, "SYS",
										player.getName() + " was affected by your request.");
							}
						} else {
							try {
								...
								activeChar.sendChatMessage(0, 0, "SYS",
										radius + " units radius affected by your request.");
				...
				if (st.countTokens() == 2) {
					...
					if (target != null) {
						...
						if (player != null) {
							...
							try {
								...
								activeChar.sendChatMessage(0, 0, "SYS",
										"Usage: //abnormal <hex_abnormal_mask> [player|radius]");
							}
						}
					}
				} else if (st.countTokens() == 1) {
					...
					if (obj != null) {
						if (performAbnormal(abnormal, obj)) {
							activeChar.sendChatMessage(0, 0, "SYS",
									obj.getName() + "'s abnormal status was affected by your request.");
						} else {
							activeChar.sendPacket(new SystemMessage(SystemMessageId.NOTHING_HAPPENED));
						}
					}
			...
			try {
				...
				if (obj != null) {
					if (!(obj instanceof L2Character)) {
						...
						activeChar.sendChatMessage(0, 0, "SYS",
								obj.getName() + " performs MSU " + skill + "/" + level + " by your request.");
					}
				}
			} catch (Exception e) {
