		...
		switch (itemId) {
			...
			case 8627:
				if ((itemId == 8622 && activeChar.getExpertiseIndex() == 0)
					...
					activeChar
							.sendPacket(new SystemMessage(SystemMessageId.INCOMPATIBLE_ITEM_GRADE).addItemName(itemId));
					return;
				}
				break;
			case 8628:
	...
	public boolean usePotion(L2Playable activeChar, int magicId, int level) {
		...
		if (skill != null) {
			...
			if (activeChar instanceof L2PcInstance) {
				...
				// Summons should be affected by herbs too, self time effect is handled at
				// L2Effect constructor
