 ...
 */
package com.gameserver.model.spawn;
...
public class AutoSpawn {
	...
	public boolean removeSpawn(AutoSpawnInstance spawnInst) {
		...
		try {
			_registeredSpawns.remove(spawnInst._objectId);
			...
			_log.error("AutoSpawnHandler: Could not auto spawn for NPC ID " + spawnInst._npcId + " (Object ID = "
		...
		if (isSpawnRegistered(objectId)) {
			...
			if (isActive) {
				...
				if (spawnInst._desDelay > 0) {
					spawnTask = ThreadPoolManager.getInstance().scheduleEffectAtFixedRate(rs, spawnInst._initDelay,
							spawnInst._resDelay);
				} else {
			...
			try {
				...
				if (spawnInst.isBroadcasting()) {
					Announcements.getInstance()
							.announceToAll("The " + npcInst.getName() + " has spawned near " + nearestTown + "!");
				}
				...
				_log.error("AutoSpawnHandler: An error occurred while initializing spawn instance (Object ID = "
						+ _objectId + ")", e);
			}
		}
	}
	...
	private class AutoDespawner implements Runnable {
		...
		public void run() {
			try {
				...
				_log.error("AutoSpawnHandler: An error occurred while despawning spawn (Object ID = " + _objectId + ")",
						e);
			}
		}
	}
