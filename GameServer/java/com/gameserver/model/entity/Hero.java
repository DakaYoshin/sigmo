...
public class Hero {
	...
	private static final String UPDATE_HERO = "UPDATE heroes SET count = ?, played = ?, active = ?"
			+ " WHERE char_id = ?";
	...
	public synchronized void computeNewHeroes(List<StatsSet> newHeroes) {
		updateHeroes(true);
		List<Integer> heroItems = new FastList<Integer>();
		for (int i : _heroItems) {
			heroItems.add(i);
		}
	...
	public void loadDiary(int charId) {
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
			...
			_log.info("Hero System: Loaded " + diaryentries + " diary entries for Hero: "
	...
	public void activateHero(L2PcInstance player) {
		...
		if (player.getClan() != null && player.getClan().getLevel() >= 5) {
			...
			clan.broadcastToOnlineMembers(
					new SystemMessage(SystemMessageId.CLAN_MEMBER_S1_BECAME_HERO_AND_GAINED_S2_REPUTATION_POINTS)
							.addString(name).addNumber(1000));
		} else {
			player.broadcastUserInfo();
		}
		updateHeroes(false);
