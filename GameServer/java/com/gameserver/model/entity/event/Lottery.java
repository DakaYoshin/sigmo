 ...
 */
package com.gameserver.model.entity.event;
...
public class Lottery
{
	...
	{
		_number = 1;
		_prize = Config.ALT_LOTTERY_PRICE;
	...
	public void increasePrize(int count)
	{
		...
		{
			_log.error("Lottery: Could not increase current lottery prize", e);
		}
		finally
		{
	...
	private class startLottery implements Runnable
	{
		...
		public void run()
		{
			...
			{
				_log.error("Lottery: Could not restore lottery data", e);
			}
			finally
			{
				ResourceUtil.closeConnection(con);
			...
			{
				_log.error("Lottery: Could not store new lottery data", e);
			}
			finally
			{
				ResourceUtil.closeConnection(con);
	...
	private class finishLottery implements Runnable
	{
		...
		public void run()
		{
			...
			{
				_log.error("Lottery: Could restore lottery data", e);
			}
			finally
			{
				ResourceUtil.closeConnection(con);
			...
			if(count1 > 0)
			{
				Announcements.getInstance().announceToAll(new SystemMessage(SystemMessageId.AMOUNT_FOR_WINNER_S1_IS_S2_ADENA_WE_HAVE_S3_PRIZE_WINNER).addNumber(getId()).addNumber(getPrize()).addNumber(count1));
			}
			else
			{
				Announcements.getInstance().announceToAll(new SystemMessage(SystemMessageId.AMOUNT_FOR_LOTTERY_S1_IS_S2_ADENA_NO_WINNER).addNumber(getId()).addNumber(getPrize()));
			}
			...
			{
				_log.error("Lottery: Could not store finished lottery data", e);
			}
			finally
			{
				ResourceUtil.closeConnection(con);
	...
	public int[] checkTicket(int id, int enchant, int type2)
	{
		...
		{
			_log.error("Lottery: Could not check lottery ticket #" + id, e);
		}
		finally
		{
			ResourceUtil.closeConnection(con);
