 ...
 */
package com.gameserver.model.entity.siege;
...
import com.gameserver.managers.MercTicketManager;
import com.gameserver.managers.SiegeGuardManager;
import com.gameserver.managers.SiegeManager;
import com.gameserver.managers.SiegeManager.SiegeSpawn;
import com.gameserver.model.L2Clan;
import com.gameserver.model.L2ClanMember;
import com.gameserver.model.L2Object;
import com.gameserver.model.L2SiegeClan;
import com.gameserver.model.L2SiegeClan.SiegeClanType;
import com.gameserver.model.L2World;
import com.gameserver.model.actor.L2Character;
import com.gameserver.model.actor.L2Npc;
import com.gameserver.model.actor.instance.L2ArtefactInstance;
...
import com.gameserver.model.spawn.L2Spawn;
import com.gameserver.network.SystemMessageId;
import com.gameserver.network.serverpackets.PlaySound;
import com.gameserver.network.serverpackets.RelationChanged;
import com.gameserver.network.serverpackets.SiegeInfo;
...
public class Siege {
		...
		public void run() {
			...
			try {
				...
				if (timeRemaining > 3600000) {
					announceToPlayer(SystemMessage.getSystemMessage(SystemMessageId.S1_HOURS_UNTIL_SIEGE_CONCLUSION)
							.addNumber(2), true);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(_castleInst),
							timeRemaining - 3600000);
				} else if (timeRemaining <= 3600000 && timeRemaining > 600000) {
					announceToPlayer(SystemMessage.getSystemMessage(SystemMessageId.S1_MINUTES_UNTIL_SIEGE_CONCLUSION)
							.addNumber(Math.round(timeRemaining / 60000)), true);
					...
					announceToPlayer(SystemMessage.getSystemMessage(SystemMessageId.CASTLE_SIEGE_S1_SECONDS_LEFT)
							.addNumber(Math.round(timeRemaining / 1000)), true);
		...
		public void run() {
			...
			try {
				...
				if (timeRemaining > 86400000) {
					_scheduledStartSiegeTask = ThreadPoolManager.getInstance()
							.scheduleGeneral(new ScheduleStartSiegeTask(_castleInst), timeRemaining - 86400000); // Prepare
																													// task
																													// for
																													// 24
																													// before
					...
					Announcements.getInstance().announceToAll(
							SystemMessage.getSystemMessage(SystemMessageId.REGISTRATION_TERM_FOR_S1_ENDED)
									.addString(getCastle().getName()));
					_isRegistrationOver = true;
					clearSiegeWaitingClan();

					_scheduledStartSiegeTask = ThreadPoolManager.getInstance()
							.scheduleGeneral(new ScheduleStartSiegeTask(_castleInst), timeRemaining - 13600000); // Prepare
																													// task
																													// for
																													// 1
																													// hr
					...
					_scheduledStartSiegeTask = ThreadPoolManager.getInstance()
							.scheduleGeneral(new ScheduleStartSiegeTask(_castleInst), timeRemaining - 600000); // Prepare
																												// task
																												// for
																												// 10
																												// minute
					...
					_scheduledStartSiegeTask = ThreadPoolManager.getInstance()
							.scheduleGeneral(new ScheduleStartSiegeTask(_castleInst), timeRemaining - 300000); // Prepare
																												// task
																												// for 5
																												// minute
																												// left.
					...
					_scheduledStartSiegeTask = ThreadPoolManager.getInstance()
							.scheduleGeneral(new ScheduleStartSiegeTask(_castleInst), timeRemaining - 10000); // Prepare
																												// task
																												// for
																												// 10
																												// seconds
					...
					_scheduledStartSiegeTask = ThreadPoolManager.getInstance()
							.scheduleGeneral(new ScheduleStartSiegeTask(_castleInst), timeRemaining); // Prepare task
																										// for second
																										// count down
				} else {
	...
	public void endSiege() {
		if (getIsInProgress()) {
			Announcements.getInstance().announceToAll(SystemMessage
					.getSystemMessage(SystemMessageId.SIEGE_OF_S1_HAS_ENDED).addString(getCastle().getName()));
			...
			if (getCastle().getOwnerId() > 0) {
				...
				Announcements.getInstance().announceToAll(
						SystemMessage.getSystemMessage(SystemMessageId.CLAN_S1_VICTORIOUS_OVER_S2_S_SIEGE)
								.addString(clan.getName()).addString(getCastle().getName()));

				// Delete circlets and crown's leader for initial castle's owner (if one was
				// existing)
				if (getCastle().getInitialCastleOwner() != null && clan != getCastle().getInitialCastleOwner()) {
					if (Config.REMOVE_CASTLE_CIRCLETS)
						CastleManager.getInstance().removeCirclet(getCastle().getInitialCastleOwner(),
								getCastle().getCastleId());
				...
				Announcements.getInstance().announceToAll(
						SystemMessage.getSystemMessage(SystemMessageId.SIEGE_S1_DRAW).addString(getCastle().getName()));
	...
	public void startSiege() {
		if (!getIsInProgress()) {
			...
			Announcements.getInstance().announceToAll(SystemMessage
					.getSystemMessage(SystemMessageId.SIEGE_OF_S1_HAS_STARTED).addString(getCastle().getName()));
		...
		for (L2SiegeClan siegeclan : getAttackerClans()) {
			...
			for (L2PcInstance member : clan.getOnlineMembers("")) {
				...
				for (L2PcInstance player : member.getKnownList().getKnownPlayers().values()) {
					player.sendPacket(
							new RelationChanged(member, member.getRelation(player), member.isAutoAttackable(player)));
	...
	public void clearSiegeWaitingClan() {
		...
		try {
			...
			PreparedStatement statement = con
					.prepareStatement("DELETE FROM siege_clans WHERE castle_id = ? AND type = 2");
			statement.setInt(1, getCastle().getCastleId());
			statement.execute();
			statement.close();
			statement = null;
	...
	public void killedCT(L2Npc ct) {
		...
		if (_controlTowerMaxCount > 0) {
			if (_controlTowerCount == 0)
				_defenderRespawnDelayPenalty = _controlTowerMaxCount
						* SiegeManager.getInstance().getControlTowerLosePenalty();
			else
				_defenderRespawnDelayPenalty = (_controlTowerMaxCount - _controlTowerCount) / _controlTowerCount
						* SiegeManager.getInstance().getControlTowerLosePenalty();
		} else {
			_defenderRespawnDelayPenalty = 0;
		}
	...
	private boolean checkIfCanRegister(L2PcInstance player) {
		if (getIsRegistrationOver()) {
			...
			player.sendMessage("Only clans with Level " + SiegeManager.getInstance().getSiegeClanMinLevel()
			...
			player.sendPacket(
					new SystemMessage(SystemMessageId.CLAN_THAT_OWNS_CASTLE_IS_AUTOMATICALLY_REGISTERED_DEFENDING));
	...
	private boolean checkIfAlreadyRegisteredForAnotherSiege(L2Clan clan) {
		for (Siege siege : SiegeManager.getInstance().getSieges()) {
			...
			// if(siege.getSiegeDate().get(Calendar.DAY_OF_WEEK) ==
	...
	public void saveSiegeDate() {
		...
		try {
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con
					.prepareStatement("Update castle set siegeDate = ?, regTimeEnd = ?, regTimeOver = ? where id = ?");
	...
	private void saveSiegeClan(L2Clan clan, int typeId, boolean isUpdateRegistration) {
		...
		try {
			if (typeId == 0 || typeId == 2 || typeId == -1) {
				if (getDefenderClans().size() + getDefenderWaitingClans().size() >= SiegeManager.getInstance()
						.getDefenderMaxClans()) {
					return;
				}
			} else {
	...
	public void setNextSiegeDate() {
		// Copy of siege date. All modifications are made on it, then once ended, it is
		// registered.
		...
		while (siegeDate.getTimeInMillis() < Calendar.getInstance().getTimeInMillis()) {
			// If current day is another than Saturday or Sunday, change it accordingly to
			// castle
			if (siegeDate.get(Calendar.DAY_OF_WEEK) != Calendar.SATURDAY
					|| siegeDate.get(Calendar.DAY_OF_WEEK) != Calendar.SUNDAY) {
		...
		// After all modifications are applied on local variable, register the time as
		// siege date of that castle.
		getCastle().getSiegeDate().setTimeInMillis(siegeDate.getTimeInMillis());

		// Send message and allow registration for next siege.
		Announcements.getInstance().announceToAll(SystemMessage
				.getSystemMessage(SystemMessageId.S1_ANNOUNCED_SIEGE_TIME).addString(getCastle().getName()));
	...
	private void spawnArtifact(int Id) {
		...
		for (SiegeSpawn _sp : SiegeManager.getInstance().getArtefactSpawnList(Id)) {
			...
			art = new L2ArtefactInstance(IdFactory.getInstance().getNextId(),
					NpcTable.getInstance().getTemplate(_sp.getNpcId()));
