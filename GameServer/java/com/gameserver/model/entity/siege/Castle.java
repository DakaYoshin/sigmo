...
import com.gameserver.managers.CastleManorManager.CropProcure;
import com.gameserver.managers.CastleManorManager.SeedProduction;
...
import com.gameserver.model.zone.type.L2CastleTeleportZone;
import com.gameserver.model.zone.type.L2CastleZone;
...
import com.gameserver.updaters.CastleUpdater;
import com.util.ResourceUtil;
import com.util.database.L2DatabaseFactory;

public class Castle {
	private final static Logger _log = LoggerFactory.getLogger(Castle.class);
	...
	public void addToTreasury(int amount) {
		...
		if (!_name.equalsIgnoreCase("aden") && !_name.equalsIgnoreCase("Rune") && !_name.equalsIgnoreCase("Schuttgart")
				&& !_name.equalsIgnoreCase("Goddard")) {
	...
	public void loadDoorUpgrade() {
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
			...
			PreparedStatement statement = con
					.prepareStatement("SELECT * FROM castle_doorupgrade WHERE doorId IN (" + sb.toString() + ")");
			ResultSet rs = statement.executeQuery();
			...
			_log.warn("Error loading door upgrade.", e);
		}
	}
	...
	private void removeDoorUpgrade() {
		...
		try {
			...
			PreparedStatement statement = con.prepareStatement(
					"DELETE FROM castle_doorupgrade WHERE doorId IN (SELECT id FROM castle_door WHERE castleId = ?)");
			statement.setInt(1, getCastleId());
			statement.execute();
			statement.close();
			statement = null;
	...
	private static void saveDoorUpgrade(int doorId, int hp) {
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
			PreparedStatement statement = con
					.prepareStatement("REPLACE INTO castle_doorupgrade (doorId, hp) VALUES (?,?)");
			statement.setInt(1, doorId);
			statement.setInt(2, hp);
			statement.execute();
			statement.close();
		} catch (Exception e) {
			_log.warn("Error saving door upgrade.", e);
		}
	}
		...
		try {
			...
			if (_procure != null) {
				...
				for (CropProcure cp : _procure) {
					values[count] = "(" + getCastleId() + "," + cp.getId() + "," + cp.getAmount() + ","
							+ cp.getStartAmount() + "," + cp.getPrice() + "," + cp.getReward() + ","
	...
	public void updateClansReputation() {
		...
		if (_formerOwner != null) {
			if (_formerOwner != ClanTable.getInstance().getClan(getOwnerId())) {
				...
				// Defenders fail
				msg = SystemMessage
						.getSystemMessage(SystemMessageId.CLAN_WAS_DEFEATED_IN_SIEGE_AND_LOST_S1_REPUTATION_POINTS);
				msg.addNumber(1000);
				...
				if (owner != null) {
					...
					msg = SystemMessage
							.getSystemMessage(SystemMessageId.CLAN_VICTORIOUS_IN_SIEGE_AND_GAINED_S1_REPUTATION_POINTS);
					msg.addNumber(1000);
