...
public final class L2FestivalGuideInstance extends L2NpcInstance
{
		...
		else if(command.startsWith("Festival"))
		{
			...
			switch(val)
			{
				case 1:
					...
					if(playerParty.getMemberCount() < Config.FESTIVAL_MIN_PLAYER)
					{
						showChatWindow(player, 2, "b", false);
						return;
					}
				...
				case 3:
					...
					player.sendPacket(new SystemMessage(SystemMessageId.CONTRIB_SCORE_INCREASED).addNumber(offeringScore));
	...
	private void showChatWindow(L2PcInstance player, int val, String suffix, boolean isDescription)
	{
		...
		{
			html.replace("%minFestivalPartyMembers%", String.valueOf(Config.FESTIVAL_MIN_PLAYER));
		}
