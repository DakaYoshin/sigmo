...
public class L2FlameTowerInstance extends L2Npc {
	...
	public void onAction(L2PcInstance player) {
		...
		if (this != player.getTarget()) {
			...
			// Send a Server->Client packet StatusUpdate of the L2Npc to the L2PcInstance to
			// update its HP bar
			...
			// Send a Server->Client packet ValidateLocation to correct the L2Npc position
			// and heading on the client
			player.sendPacket(new ValidateLocation(this));
		} else {
			if (isAutoAttackable(player) && Math.abs(player.getZ() - getZ()) < 100 // Less then max height difference,
																					// delete check when geo
					&& GeoData.getInstance().canSeeTarget(player, this)) {
				// Notify the L2PcInstance AI with AI_INTENTION_INTERACT
				player.getAI().setIntention(CtrlIntention.AI_INTENTION_ATTACK, this);
			}
		}
		// Send a Server->Client ActionFailed to the L2PcInstance in order to avoid that
		// the client wait another packet
