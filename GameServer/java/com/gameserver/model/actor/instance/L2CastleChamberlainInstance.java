...
import com.gameserver.network.serverpackets.ExShowCropInfo;
import com.gameserver.network.serverpackets.ExShowCropSetting;
import com.gameserver.network.serverpackets.ExShowManorDefaultInfo;
import com.gameserver.network.serverpackets.ExShowSeedInfo;
import com.gameserver.network.serverpackets.ExShowSeedSetting;
...
public class L2CastleChamberlainInstance extends L2MerchantInstance {
	...
	public void onBypassFeedback(L2PcInstance player, String command) {
			...
			switch (ask) {
				case 3: // Current seeds (Manor info)
					if (time == 1 && !CastleManager.getInstance().getCastleById(castleId).isNextPeriodApproved())
						...
						player.sendPacket(new ExShowSeedInfo(castleId,
				...
				case 4: // Current crops (Manor info)
					if (time == 1 && !CastleManager.getInstance().getCastleById(castleId).isNextPeriodApproved())
						...
						player.sendPacket(new ExShowCropInfo(castleId,
				...
				case 7: // Edit seed setup
					if (castle.isNextPeriodApproved())
						player.sendPacket(
								new SystemMessage(SystemMessageId.A_MANOR_CANNOT_BE_SET_UP_BETWEEN_6_AM_AND_8_PM));
					else
		...
		} else if (actualCommand.equalsIgnoreCase("siege_change")) // set siege time
		{
			...
			if (castle.getSiege().getTimeRegistrationOverDate().getTimeInMillis() < Calendar.getInstance()
					.getTimeInMillis())
		...
		} else if (actualCommand.equalsIgnoreCase("siege_time_set")) // set preDay
		{
			...
			if (_preHour != 6) {
				...
				castle.getSiege().setNextSiegeDate();
			...
			if (cond == COND_OWNER) {
				if (player.getInventory().getItemByItemId(6841) == null) {
					 ...
					 * EARNED_ITEM_S1).addItemName(6841));
					 */
			...
			if (player.getClan().getWarehouse().getAdena() < price) {
				player.sendPacket(new SystemMessage(SystemMessageId.NOT_ENOUGH_ADENA_IN_CWH));
				return;
			}
	...
	/**
	 * Retrieve the price of the door, following its type, required level of upgrade
	 * and current Seven Signs state.
	 * 
	 ...
	 * @return The price modified by Seal of Strife state (-20% if Dawn is winning,
	 *         x3 if Dusk is winning).
	 */
