 ...
 */
package com.gameserver.model.quest;
...
import com.gameserver.scripting.ManagedScript;
import com.gameserver.scripting.ScriptManager;
...
public class Quest extends ManagedScript {
	...
	public static enum QuestEventType {
		NPC_FIRST_TALK(false), // control the first dialog shown by NPCs when they are clicked (some quests
								// must override the default npc action)
	 ...
	 * 
	 * @param name:       name of the timer (also passed back as "event" in
	 *                    onAdvEvent)
	 * @param time:       time in ms for when to fire the timer
	 * @param npc:        npc associated with this timer (can be null)
	 * @param player:     player associated with this timer (can be null)
	...
	public final boolean notifySkillSee(L2Npc npc, L2PcInstance caster, L2Skill skill, L2Object[] targets,
			boolean isPet) {
	...
	public final static void playerEnter(L2PcInstance player) {
		...
		try {
			...
			PreparedStatement invalidQuestData = con
					.prepareStatement("DELETE FROM character_quests WHERE char_id = ? and name = ?");
			PreparedStatement invalidQuestDataVar = con
					.prepareStatement("DELETE FROM character_quests WHERE char_id = ? and name = ? and var = ?");
			...
			while (rs.next()) {
				...
				if (state == null) {
					_log.warn("Unknown state in quest " + questId + " for player " + player.getName());
					if (Config.AUTODELETE_INVALID_QUEST_DATA) {
						invalidQuestData.setInt(1, player.getObjectId());
						invalidQuestData.setString(2, questId);
						invalidQuestData.executeUpdate();
	...
	public final void saveGlobalQuestVar(String var, String value) {
		...
		try {
			...
			PreparedStatement statement;
			statement = con
					.prepareStatement("REPLACE INTO quest_global_data (quest_name, var, value) VALUES (?, ?, ?)");
			...
			_log.error("could not insert global quest variable", e);
		} finally {
			ResourceUtil.closeConnection(con);
		}
	}
	...
	public final String loadGlobalQuestVar(String var) {
		...
		try {
			...
			_log.error("could not load global quest variable", e);
		} finally {
			ResourceUtil.closeConnection(con);
		}
		return result;
	...
	public final void deleteGlobalQuestVar(String var) {
		...
		try {
			...
			_log.error("could not delete global quest variable", e);
		} finally {
			ResourceUtil.closeConnection(con);
		}
	}
	...
	public static void createQuestVarInDb(QuestState qs, String var, String value) {
		...
		try {
			...
			statement = con.prepareStatement(
					"INSERT INTO character_quests (char_id, name, var, value) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE value=?");
	...
	public static void updateQuestVarInDb(QuestState qs, String var, String value) {
		...
		try {
			...
			PreparedStatement statement;
			statement = con.prepareStatement(
					"UPDATE character_quests SET value = ? WHERE char_id = ? AND name = ? AND var = ?");
	 ...
	 * <LI>Test if quest is completed. If true, add a star (*) before the ID
	 * state</LI>
	 * <LI>Save in database the ID state (with or without the star) for the variable
	...
	/**
	 * Return default html page "You are either not on a quest that involves this
	 * NPC.."
	 * 
	 * @param player
	 * @return
