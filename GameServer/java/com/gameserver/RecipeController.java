...
import com.gameserver.model.L2ManufactureItem;
...
import com.gameserver.network.serverpackets.RecipeBookItemList;
import com.gameserver.network.serverpackets.RecipeItemMakeInfo;
import com.gameserver.network.serverpackets.RecipeShopItemInfo;
...
public class RecipeController {
	...
	protected static final Map<L2PcInstance, RecipeItemMaker> _activeMakers = Collections
			.synchronizedMap(new WeakHashMap<L2PcInstance, RecipeItemMaker>());
	...
	public synchronized void requestManufactureItem(L2PcInstance manufacturer, int recipeListId, L2PcInstance player) {
		...
		if (!dwarfRecipes.contains(recipeList) && !commonRecipes.contains(recipeList)) {
			Util.handleIllegalPlayerAction(player, "Warning!! Character " + player.getName() + " of account "
					+ player.getAccountName() + " sent a false recipe id.", Config.DEFAULT_PUNISH);
			commonRecipes = null;
			dwarfRecipes = null;
			return;
		}
	...
	public synchronized void requestMakeItem(L2PcInstance player, int recipeListId) {
		...
		if (Config.ALT_GAME_CREATION && (maker = _activeMakers.get(player)) != null) {
			player.sendPacket(new SystemMessage(SystemMessageId.S1_S2).addString("You are busy creating ")
					.addItemName(recipeList.getItemId()));
			return;
		}
	...
	private class RecipeItemMaker implements Runnable {
		...
		private void finishCrafting() {
			...
			if (_target != _player && _price > 0) {
				L2ItemInstance adenatransfer = _target.transferItem("PayManufacture",
						_target.getInventory().getAdenaInstance().getObjectId(), _price, _player.getInventory(),
						_player);
					...
					if (item == null || item.getCount() < quantity) {
						_target.sendMessage("You dont have the right elements for making this item"
								+ (_recipeList.isConsumable() && Config.RATE_CONSUMABLE_COST != 1
										? ".\nDue to server rates you need " + Config.RATE_CONSUMABLE_COST
		...
		private void rewardPlayer() {
			...
			L2ItemInstance createdItem = _target.getInventory().addItem("Manufacture", itemId, itemCount, _target,
					_player);

			if (itemCount > 1) {
				_target.sendPacket(
						new SystemMessage(SystemMessageId.EARNED_S2_S1_S).addItemName(itemId).addNumber(itemCount));
			} else {
			...
			if (Config.ALT_GAME_CREATION) {
				...
				_player.addExpAndSp(
						(int) _player.calcStat(Stats.EXPSP_RATE,
