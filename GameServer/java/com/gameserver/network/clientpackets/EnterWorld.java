...
import com.gameserver.datatables.GMSkillTable;
...
import com.gameserver.managers.ClanHallManager;
...
import com.gameserver.managers.PetitionManager;
import com.gameserver.managers.SiegeManager;
import com.gameserver.model.CursedWeapon;
...
import com.gameserver.model.actor.instance.L2ClassMasterInstance;
...
import com.gameserver.model.entity.ClanHall;
import com.gameserver.model.entity.Hero;
import com.gameserver.model.entity.Wedding;
...
import com.gameserver.network.serverpackets.ClientSetTime;
import com.gameserver.network.serverpackets.Die;
import com.gameserver.network.serverpackets.Earthquake;
...
import com.gameserver.network.serverpackets.PledgeShowMemberListAll;
import com.gameserver.network.serverpackets.PledgeShowMemberListUpdate;
import com.gameserver.network.serverpackets.PledgeSkillList;
import com.gameserver.network.serverpackets.PledgeStatusChanged;
import com.gameserver.network.serverpackets.QuestList;
import com.gameserver.network.serverpackets.ShortCutInit;
import com.gameserver.network.serverpackets.SignsSky;
...
public class EnterWorld extends L2GameClientPacket {
	...
	protected void runImpl() {
					...
					if (i.isEquipable()) {
						if (i.getEnchantLevel() > Config.MAX_ITEM_ENCHANT_KICK) {
							...
							sendPacket(new ExShowScreenMessage(
									" You have an over enchanted item, you will be kicked from server! ", 6000));
							Util.handleIllegalPlayerAction(activeChar,
									"Player " + activeChar.getName() + " has Overenchanted  item! Kicked! ",
		...
		if (!activeChar.isGM()) {
			if (activeChar.getName().length() < 3 || activeChar.getName().length() > 16
					|| !Util.isAlphaNumeric(activeChar.getName()) || !isValidName(activeChar.getName())) {
		...
		if (Olympiad.getInstance().playerInStadia(activeChar)) {
			...
			activeChar.sendMessage(
					"You have been teleported to the nearest town due to you being in an Olympiad Stadium.");
		}
		...
		try {
			// Sets the apropriate Pledge Class for the clannie (e.g. Viscount, Count,
			// Baron, Marquiz)
		...
		if (Config.PLAYERS_ONLINE_LOGIN) {
			activeChar.sendMessage((new StringBuilder()).append("Players online: ")
					.append(L2World.getInstance().getAllPlayers().size() + Config.PLAYERS_ONLINE_TRICK).toString());
		}
	...
	private void gmEnter(L2PcInstance activeChar) {
		if (activeChar.isGM()) {
			...
			if (Config.GM_STARTUP_INVISIBLE && AdminCommandAccessRights.getInstance().hasAccess("admin_invisible",
	...
	private void ColorSystem(L2PcInstance activeChar) {
		...
		if (activeChar.getClan() != null && activeChar.isClanLeader() && Config.CLAN_LEADER_COLOR_ENABLED
	...
	private void verifyAndLoadShots(L2PcInstance activeChar) {
		...
		if (!activeChar.isDead() && activeChar.getActiveWeaponItem() != null) {
			...
			if ((soulId > -1)
					&& activeChar.getInventory().getInventoryItemCount(soulId, -1) > Config.AUTO_ACTIVATE_SHOTS_MIN) {
				activeChar.addAutoSoulShot(soulId);
				activeChar.sendPacket(new ExAutoSoulShot(soulId, 1));
				activeChar.sendPacket(new SystemMessage(SystemMessageId.USE_OF_S1_WILL_BE_AUTO)
						.addItemName(activeChar.getInventory().getItemByItemId(soulId)));
			}

			if ((bspiritId > -1) && activeChar.getInventory().getInventoryItemCount(bspiritId,
					-1) > Config.AUTO_ACTIVATE_SHOTS_MIN) {
				activeChar.addAutoSoulShot(bspiritId);
				activeChar.sendPacket(new ExAutoSoulShot(bspiritId, 1));
				activeChar.sendPacket(new SystemMessage(SystemMessageId.USE_OF_S1_WILL_BE_AUTO)
						.addItemName(activeChar.getInventory().getItemByItemId(bspiritId)));
			} else if ((spiritId > -1)
					&& activeChar.getInventory().getInventoryItemCount(spiritId, -1) > Config.AUTO_ACTIVATE_SHOTS_MIN) {
				activeChar.addAutoSoulShot(spiritId);
				activeChar.sendPacket(new ExAutoSoulShot(spiritId, 1));
				activeChar.sendPacket(new SystemMessage(SystemMessageId.USE_OF_S1_WILL_BE_AUTO)
						.addItemName(activeChar.getInventory().getItemByItemId(spiritId)));
			}
		...
		try {
			...
			while (rset.next()) {
				...
				if (friend != null) {
					...
					friend.sendPacket(
							new SystemMessage(SystemMessageId.FRIEND_S1_HAS_LOGGED_IN).addString(cha.getName()));
				}
			}
	...
	private void notifyClanMembers(L2PcInstance activeChar) {
		...
		if (clan != null) {
			...
			clan.broadcastToOtherOnlineMembers(
					(new SystemMessage(SystemMessageId.CLAN_MEMBER_S1_LOGGED_IN).addString(activeChar.getName())),
					activeChar);
	...
	private void notifySponsorOrApprentice(L2PcInstance activeChar) {
		if (activeChar.getSponsor() != 0) {
			...
			if (sponsor != null) {
				sponsor.sendPacket(new SystemMessage(SystemMessageId.YOUR_APPRENTICE_S1_HAS_LOGGED_IN)
						.addString(activeChar.getName()));
			}
		} else if (activeChar.getApprentice() != 0) {
			L2PcInstance apprentice = (L2PcInstance) L2World.getInstance().findObject(activeChar.getApprentice());

			if (apprentice != null) {
				apprentice.sendPacket(new SystemMessage(SystemMessageId.YOUR_SPONSOR_S1_HAS_LOGGED_IN)
						.addString(activeChar.getName()));
			}
		}
	}
	...
	public static void warnAllPlayers() {
		for (L2PcInstance player : _onlineplayers) {
			String file = "data/html/xline/chaos/warning.htm";
		...
		if (clan != null) {
			if (clan.getHasCastle() > 0) {
				...
				if ((castle != null) && (activeChar.getObjectId() == clan.getLeaderId()))
					Announcements.getInstance().announceToAll("Lord " + activeChar.getName() + " Ruler Of "
							+ castle.getName() + " Castle is Now Online!");
			}
		}
	}
