...
import com.gameserver.network.clientpackets.*;
import com.mmocore.IClientFactory;
import com.mmocore.IMMOExecutor;
import com.mmocore.IPacketHandler;
...
public final class L2GamePacketHandler implements IPacketHandler<L2GameClient>, IClientFactory<L2GameClient>, IMMOExecutor<L2GameClient>
{
	...
	private void UnknownPunish(L2GameClient client)
	{
		switch(Config.UNKNOWN_PACKETS_PUNiSHMENT)
		{
			...
			case 2:
				_log.warning("PacketProtection: " + client.toString() + " got kicked due to flooding of unknown packets");

				if(client.getActiveChar() != null)
				{
					GmListTable.broadcastMessageToGMs("Player " + client.getActiveChar().toString() + " flooding unknown packets and got kicked.");
					client.getActiveChar().sendMessage("You are kicked for unknown packet flooding, GM informed");
					client.getActiveChar().closeNetConnection();
				}
				break;

			case 3:
				_log.warning("PacketProtection: " + client.toString() + " got banned due to flooding of unknown packets");
