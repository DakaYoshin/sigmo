...
public class Disablers implements ISkillHandler {
	...
	protected static final Logger _log = LoggerFactory.getLogger(Disablers.class);
		...
		if (activeChar instanceof L2PcInstance) {
			if (weaponInst == null && skill.isOffensive()) {
				activeChar.sendPacket(new SystemMessage(SystemMessageId.S1_S2)
						.addString("You must equip a weapon before casting a spell."));
				return;
			}
		}
			...
			switch (type) {
				case BETRAY: {
					if (Formulas.getInstance().calcSkillSuccess(activeChar, target, skill, ss, sps, bss)) {
						...
						activeChar.sendPacket(new SystemMessage(SystemMessageId.S1_WAS_UNAFFECTED_BY_S2)
								.addString(target.getName()).addSkillName(skill.getId()));
					}
					break;
				}
				case FAKE_DEATH: {
				...
				case STUN: {
					...
					if (Formulas.getInstance().calcSkillSuccess(activeChar, target, skill, ss, sps, bss)) {
						...
						if (activeChar instanceof L2PcInstance) {
							activeChar.sendPacket(new SystemMessage(SystemMessageId.S1_WAS_UNAFFECTED_BY_S2)
									.addString(target.getName()).addSkillName(skill.getDisplayId()));
						}
					}
				...
				case AGGREDUCE: {
					if (target instanceof L2Attackable) {
						...
						double aggdiff = ((L2Attackable) target).getHating(activeChar) - target.calcStat(
								Stats.AGGRESSION, ((L2Attackable) target).getHating(activeChar), target, skill);
						...
						if (Rnd.get(100) < landrate) {
							...
							for (L2Effect e : effects) {
								...
								if (e.getSkill().getId() != 4082 && e.getSkill().getId() != 4215
										&& e.getSkill().getId() != 5182 && e.getSkill().getId() != 4515
										&& e.getSkill().getId() != 110 && e.getSkill().getId() != 111
										&& e.getSkill().getId() != 1323 && e.getSkill().getId() != 1325
										&& !skill.isToggle()) {
				...
				case NEGATE: {
					if (skill.getId() == 2275) {
						...
						for (String stat : _negateStats) {
							...
							if (stat == "heal") {
								ISkillHandler Healhandler = SkillHandler.getInstance()
										.getSkillHandler(L2SkillType.HEAL);
								if (Healhandler == null) {
								...
								try {
									...
									_log.warn("Error using HEAL handler: " + e.getMessage(), e);
								}
							}
						}
					}
