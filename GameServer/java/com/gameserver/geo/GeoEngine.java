...
import com.gameserver.geo.pathfinding.cellnodes.CellPathFinding;
import com.gameserver.geo.util.Door;
import com.gameserver.geo.util.L2Arrays;
import com.gameserver.geo.util.LookupTable;
...
public final class GeoEngine extends GeoData {
	...
	public boolean canSeeTarget(L2Object cha, Point3D target) {
		...
		if (Door.getInstance().checkIfDoorsBetween(cha.getX(), cha.getY(), cha.getZ(), target.getX(), target.getY(),
				target.getZ())) {
			return false;
		}
	...
	public boolean canSeeTargetDebug(L2PcInstance gm, L2Object target) {
		...
		if (z >= z2) {
			...
			return canSeeDebug(gm, (target.getX() - L2World.MAP_MIN_X) >> 4, (target.getY() - L2World.MAP_MIN_Y) >> 4,
					z2, (gm.getX() - L2World.MAP_MIN_X) >> 4, (gm.getY() - L2World.MAP_MIN_Y) >> 4, z);
		}
	}
	...
	private short nGetHeight(int geox, int geoy, int z) {
		...
		if (type == 0) {
			...
			if (layers <= 0 || layers > 125) {
				_log.warn("Broken geofile (case1), region: " + region + " - invalid layer count: " + layers + " at: "
						+ geox + " " + geoy);
				return (short) z;
			}
	...
	private short nGetSpawnHeight(int geox, int geoy, int zmin, int zmax, L2Spawn spawn) {
		...
		if (type == 0) {
			...
			index++;
			if (layers <= 0 || layers > 125) {
				_log.warn("Broken geofile (case2), region: " + region + " - invalid layer count: " + layers + " at: "
						+ geox + " " + geoy);
				return (short) zmin;
			}
			while (layers > 0) {
				...
				if (_log.isDebugEnabled()) {
					_log.warn(
							"SpawnHeight Error - Couldnt find correct layer to spawn NPC - GeoData or Spawnlist Bug!: zmin: "
									+ zmin + " zmax: " + zmax + " value: " + temph + " SpawnId: " + spawn + " at: "
									+ geox + " : " + geoy);
				}
				return (short) zmin;
			}
		...
		if (temph > zmax + 1000 || temph < zmin - 1000) {
			if (_log.isDebugEnabled()) {
				_log.warn("SpawnHeight Error - Spawnlist z value is wrong or GeoData error: zmin: " + zmin + " zmax: "
						+ zmax + " value: " + temph + " SpawnId: " + spawn + " at: " + geox + " : " + geoy);
			}
			return (short) zmin;
		}
		return temph;
	...
	private double nCanMoveNext(int x, int y, int z, int tx, int ty, int tz) {
		...
		if (type == 0) {
			...
			short height = -1;
			if (layers <= 0 || layers > 125) {
				_log.warn("Broken geofile (case3), region: " + region + " - invalid layer count: " + layers + " at: "
						+ x + " " + y);
				return z;
			}
	...
	private boolean nLOS(int x, int y, int z, int inc_x, int inc_y, double inc_z, int tz, boolean debug) {
		...
		if (type == 0) {
			...
			short tempZ = -1;
			if (layers <= 0 || layers > 125) {
				_log.warn("Broken geofile (case4), region: " + region + " - invalid layer count: " + layers + " at: "
						+ x + " " + y);
				return false;
			}
			...
			if (debug) {
				_log.warn("z:" + z + " x: " + cellX + " y:" + cellY + " la " + layers + " lo:" + lowerHeight + " up:"
						+ upperHeight);
			}
			...
			if (!highestlayer) {
				if (!checkNSWE(NSWE, x, y, x + inc_x, y + inc_y)) {
					if (debug) {
						_log.warn("block and next in x" + inc_x + " y" + inc_y + " is:"
	...
	private short nGetNSWE(int x, int y, int z) {
		...
		if (type == 0) {
			...
			short height = -1;
			if (layers <= 0 || layers > 125) {
				_log.warn("Broken geofile (case5), region: " + region + " - invalid layer count: " + layers + " at: "
						+ x + " " + y);
				return 15;
			}
			short tempz = Short.MIN_VALUE;
