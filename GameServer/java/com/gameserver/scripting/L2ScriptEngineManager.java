...
public final class L2ScriptEngineManager {
	...
	private L2ScriptEngineManager() {
		...
		for (ScriptEngineFactory factory : factories) {
			try {
				...
				_log.error("Failed initializing factory.", e);
			}
		}
	...
	public void executeScriptsList(File list) throws IOException {
		if (list.isFile()) {
			try (LineNumberReader lnr = new LineNumberReader(new InputStreamReader(new FileInputStream(list)))) {
				String line;
				File file;
				...
				if (file.isDirectory() && recurseDown && maxDepth > currentDepth) {
					...
					try {
						...
						if (e instanceof ScriptException) {
							reportScriptFileError(file, (ScriptException) e);
						} else {
							_log.error("IO error while executing script: " + file.getAbsolutePath(), e);
						}
					}
				}
			}
	...
	public CompiledScriptCache loadCompiledScriptCache() {
		if (Config.SCRIPT_CACHE) {
			File file = new File(SCRIPT_FOLDER, "CompiledScripts.dat");
			if (file.isFile()) {
		...
		if (dir != null) {
			...
			try {
				...
				try (FileOutputStream fos = new FileOutputStream(file)) {
					...
					_log.error("Failed executing script: " + script.getAbsolutePath() + ". See " + file.getName()
							+ " for details.", e);
				}
			} catch (Exception e1) {
				_log.error("Failed executing script: " + script.getAbsolutePath(), e1);
			}
		} else {
			_log.warn(
					"Failed executing script. Additionally failed when trying to write an error report on script directory: "
