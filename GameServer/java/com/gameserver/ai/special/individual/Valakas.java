 ...
 */
package com.gameserver.ai.special.individual;
...
import com.gameserver.model.quest.QuestTimer;
...
import com.gameserver.network.serverpackets.SpecialCamera;
...
public class Valakas extends Quest implements Runnable {
	...
	private static final byte WAITING = 1; // Valakas is spawend and someone has entered, triggering a 30 minute window
											// for additional people to enter
	...
	public Valakas(int id, String name, String descr) {
		...
		if (status == DEAD) {
			...
			// if valakas is locked until a certain time, mark it so and start the unlock
			// timer
			// the unlock time has not yet expired. Mark valakas as currently locked. Setup
			// a timer
			// to fire at the correct time (calculate the time between now and the unlock
			// time,
			...
			L2GrandBossInstance valakas = (L2GrandBossInstance) addSpawn(VALAKAS, loc_x, loc_y, loc_z, heading, false,
					0);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
		if (npc != null) {
			...
			if (event.equalsIgnoreCase("check_activity_and_do_actions")) {
				...
				if (status == FIGHTING && !npc.getSpawn().is_customBossInstance() // if it's a custom spawn, dnt despawn
																					// it for inactivity
			...
			if (event.equalsIgnoreCase("lock_entry_and_spawn_valakas")) {
				...
				L2GrandBossInstance valakas = (L2GrandBossInstance) addSpawn(VALAKAS, loc_x, loc_y, loc_z, heading,
						false, 0);
				GrandBossManager.getInstance().addBoss(valakas);
				...
				// L2GrandBossInstance valakas = (L2GrandBossInstance) addSpawn(VALAKAS,
				// -105200, -253104, -15264, 32768, false, 0);
				// GrandBossManager.getInstance().addBoss(valakas);
				GrandBossManager.getInstance().setBossStatus(VALAKAS, DORMANT);
			...
			if (npc.getCurrentHp() > ((npc.getMaxHp() * 1) / 4)) {
				...
				if (c2.getZ() < (npc.getZ() + 200)) {
					if (Rnd.get(100) < 20) {
						...
						if (Util.calculateDirectionTo(npc, c2) < 20)
			...
			for (L2Object obj : objs) {
				if (obj instanceof L2PcInstance) {
					if (Util.checkIfInRange(10000, _valakas, obj, true)) {
						((L2Character) obj).sendPacket(
								new PlaySound(1, "B03_A", 1, _valakas.getObjectId(), 212852, -114842, -1632));
			...
			for (L2Object obj : objs) {
				if (obj instanceof L2PcInstance || obj instanceof L2Summon) {
					if (Util.checkIfInRange(5000, npc, obj, true) && !((L2Character) obj).isDead()
							&& (obj instanceof L2PcInstance) && !((L2PcInstance) obj).isGM())
