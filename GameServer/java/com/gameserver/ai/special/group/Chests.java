 ...
 */
package com.gameserver.ai.special.group;
...
public class Chests extends L2AttackableAIScript {
	...
	public String onSkillSee(L2Npc npc, L2PcInstance caster, L2Skill skill, L2Object[] targets, boolean isPet) {
		if (npc instanceof L2ChestInstance) {
			...
			// If this chest has already been interacted, no further AI decisions are
			// needed.
				...
				if (Rnd.get(100) < IS_BOX) {
					if (skill.getId() == SKILL_DELUXE_KEY) {
						...
						// Used a key but failed to open: disappears with no rewards.
							...
							chest.doCast(SkillTable.getInstance().getInfo(4143,
									Math.min(10, Math.round(npc.getLevel() / 10))));
					} else
						chest.doCast(
								SkillTable.getInstance().getInfo(4143, Math.min(10, Math.round(npc.getLevel() / 10))));
				}
