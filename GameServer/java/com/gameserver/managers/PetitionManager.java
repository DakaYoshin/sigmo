...
public final class PetitionManager
{
	...
	private class Petition
	{
		...
		public boolean endPetitionConsultation(PetitionState endState)
		{
			...
			if(getResponder() != null && getResponder().isOnline() == 1)
			{
				...
				{
					getResponder().sendPacket(new SystemMessage(SystemMessageId.PETITION_ENDED_WITH_S1).addString(getPetitioner().getName()));

					if(endState == PetitionState.Petitioner_Cancel)
					{
						getResponder().sendPacket(new SystemMessage(SystemMessageId.RECENT_NO_S1_CANCELED).addNumber(getId()));
					}
				}
			}
	...
	public boolean acceptPetition(L2PcInstance respondingAdmin, int petitionId)
	{
		...
		currPetition.sendResponderPacket(new SystemMessage(SystemMessageId.PETITION_ACCEPTED_RECENT_NO_S1).addNumber(currPetition.getId()));

		currPetition.sendResponderPacket(new SystemMessage(SystemMessageId.PETITION_WITH_S1_UNDER_WAY).addString(currPetition.getPetitioner().getName()));
	...
	public void sendPendingPetitionList(L2PcInstance activeChar)
	{
		...
		htmlContent.append("</table><br><button value=\"Refresh\" action=\"bypass -h admin_view_petitions\" width=\"50\" " + "height=\"15\" back=\"sek.cbui94\" fore=\"sek.cbui92\"><br><font color=\"333333\" align=\"center\">_______________________________________</font><br1><font color=\"3399ff\">Server Software created by L2L2jSigmo.</font><br></center></body></html>");
