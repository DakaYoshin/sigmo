...
public class DimensionalRiftManager {
	...
	private void loadRooms() {
		...
		File f = new File("./data/xml/dimensional_rift_rooms.xml");
		if (!f.exists()) {
				...
				if (n.getNodeName().equalsIgnoreCase("list")) {
					for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling()) {
						if (d.getNodeName().equalsIgnoreCase("room")) {
							...
							boolean isBossRoom = Byte
									.valueOf(d.getAttributes().getNamedItem("boss").getNodeValue()) > 0;

							if (!_rooms.containsKey(type)) {
								_rooms.put(type, new FastMap<Byte, DimensionalRiftRoom>());
							}

							_rooms.get(type).put(room_id, new DimensionalRiftRoom(type, room_id, xMin, xMax, yMin, yMax,
	...
	public void loadSpawns() {
		...
		try {
			...
			File file = new File("./data/xml/dimensional_rift_spawns.xml");
			if (!file.exists()) {
	...
	public class DimensionalRiftRoom {
		...
		public DimensionalRiftRoom(byte type, byte room, int xMin, int xMax, int yMin, int yMax, int zMin, int zMax,
				int xT, int yT, int zT, boolean isBossRoom) {
			_type = type;
			_room = room;
			_xMin = xMin + 128;
			_xMax = xMax - 128;
