...
import com.gameserver.model.entity.event.BW;
import com.gameserver.model.entity.event.CTF;
import com.gameserver.model.entity.event.DM;
import com.gameserver.model.entity.event.FunEvent;
import com.gameserver.model.entity.event.FunEvent.State;
import com.gameserver.model.entity.event.TvT;
import com.gameserver.network.serverpackets.NpcHtmlMessage;

public class FunEventsManager {
	// private static final Logger _log =
	// LogManager.getLogger(FunEventsManager.class);
	...
	public void notifyJoinCursed(L2PcInstance player) {
		...
		if (event != null && event.getState() == State.PARTICIPATING && !event.EVENT_JOIN_CURSED) {
			...
			event.removePlayer(player);
			player.sendMessage("You have been removed from the event because you possess a cursed weapon.");
		}
	}

	public void notifyLevelChanged(L2PcInstance player) {
		FunEvent event = _events.get(player.getEventName());
		if (event != null && event.getState() == State.PARTICIPATING
				&& (player.getLevel() > event.EVENT_PLAYER_LEVEL_MAX
						|| player.getLevel() < event.EVENT_PLAYER_LEVEL_MIN)) {
			event.removePlayer(player);
			event.removePlayer(player);
			player.sendMessage(
					"You have been removed from the event because your level does not meet the requirements.");
		}
	}
	...
	public boolean notifyPlayerKilled(L2PcInstance player, L2Character killer) {
		...
		if (player.isFightingInEvent() && player.isInSameEvent(killerInstance)
				&& !player.isInSameTeam(killerInstance)) {
