...
import com.loginserver.network.serverpackets.LoginFail.LoginFailReason;
import com.util.Util;
import com.util.database.L2DatabaseFactory;
import com.util.protection.Base64;
import com.util.protection.ScrambledKeyPair;
...
public class LoginController {
	...
	public boolean isLoginPossible(L2LoginClient client, int serverId) {
		...
		if (gsi != null && gsi.isAuthed()) {
			boolean loginOk = gsi.getCurrentPlayerCount() < gsi.getMaxPlayers()
					&& gsi.getStatus() != ServerStatus.STATUS_GM_ONLY || access >= 100;
	...
	public void setAccountAccessLevel(String account, int banLevel) {
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con
						.prepareStatement("UPDATE accounts SET access_level = ? WHERE login = ?")) {
	...
	public boolean isGM(String user) {
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con
						.prepareStatement("SELECT access_level FROM accounts WHERE login = ?")) {
			statement.setString(1, user);
			try (ResultSet rset = statement.executeQuery()) {
				if (rset.next()) {
					if (rset.getInt(1) >= 100) {
						ok = true;
					}
				}
			}
	...
	public boolean loginValid(String user, String password, L2LoginClient client) {
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
			...
			try (PreparedStatement statement = con
					.prepareStatement("SELECT password, access_level, lastServer FROM accounts WHERE login = ?")) {
				statement.setString(1, user);
				try (ResultSet rset = statement.executeQuery()) {
					if (rset.next()) {
				...
				if (Config.AUTO_CREATE_ACCOUNTS) {
					if (user.length() >= 2 && user.length() <= 14) {
						try (PreparedStatement statement = con.prepareStatement(
								"INSERT INTO accounts (login, password, lastactive, access_level, lastIP) VALUES (?, ?, ?, ?, ?)")) {
							statement.setString(1, user);
			...
			if (ok) {
				...
				try (PreparedStatement statement = con
						.prepareStatement("UPDATE accounts SET lastactive = ?, lastIP = ? WHERE login = ?")) {
		...
		if (!ok) {
			...
			if (failedCount >= Config.LOGIN_TRY_BEFORE_BAN) {
				_log.info("Banning '" + address.getHostAddress() + "' for " + Config.LOGIN_BLOCK_AFTER_BAN
	...
	class PurgeThread extends Thread {
		...
		public void run() {
			for (;;) {
				...
				synchronized (_loginServerClients) {
					for (FastMap.Entry<String, L2LoginClient> e = _loginServerClients.head(),
							end = _loginServerClients.tail(); (e = e.getNext()) != end;) {
