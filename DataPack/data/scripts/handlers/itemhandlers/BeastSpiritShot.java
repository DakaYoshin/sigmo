...
public class BeastSpiritShot implements IItemHandler {
	...
	public void useItem(L2Playable playable, L2ItemInstance item) {
		...
		if (activePet.isDead()) {
			activeOwner.sendPacket(
					new SystemMessage(SystemMessageId.SOULSHOTS_AND_SPIRITSHOTS_ARE_NOT_AVAILABLE_FOR_A_DEAD_PET));
			return;
		}

		int itemId = item.getItemId();
		// boolean isBlessed = (itemId == 6647 || itemId == 20334);
		int shotConsumption = activePet.getSpiritShotsPerHit();
		...
		Broadcast.toSelfAndKnownPlayersInRadius(activeOwner, new MagicSkillUser(activePet, activePet, skillId, 1, 0, 0),
				360000/* 600 */);
	}
