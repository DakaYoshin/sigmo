...
DROPLIST={
29020: [7,RED_PIPETTE_KNIFE,50]        #Baium...NOTE: rate may need adjustment
}
...
class Quest (JQuest) :
  ...
  def onEvent (self,event,st) :
    ...
    ## FINAL ITEM EXCHANGE SECTION
      ...
      for Item in st.getPlayer().getInventory().getItems():
        ...
        if TopBGradeWeaponData.has_key(weaponType) and not Item.isEquipped() and itemTemplate.getCrystalType() == 0x03 and itemTemplate.getDuration() == -1 :
          ...
          if pDam >= TopBGradeWeaponData[weaponType][0] or mDam >= TopBGradeWeaponData[weaponType][1] :
            htmltext += "<a action=\"bypass -h Quest q234_FatesWhisper selectBGrade_" + str(Item.getObjectId()) +"\">" + itemTemplate.getName() + "+" + str(Item.getEnchantLevel()) + "</a><br>"
      if htmltext == "": 
      ...
      if Item and not Item.isEquipped() :
        st.getPlayer().destroyItem("q234_FatesWhisper",bGradeObjId, 1, st.getPlayer(), 0)
        st.set("bypass","1")
    ...
    # if quest is accepted and in progress
      ...
      ## KASPAR
        ...
        if cond==7 and st.getQuestItemsCount(PIPETTE_KNIFE) and not st.getQuestItemsCount(RED_PIPETTE_KNIFE) :
          if st.getQuestItemsCount(BLOODED_FABRIC) >= 9:
            htmltext = "30833-03.htm"  # great! Here is your mold for Leorin
            st.takeItems(BLOODED_FABRIC,9)
  ...
  def onAttack (self, npc, player, damage, isPet):                   
    ...
    if value == st.getInt("cond") and npcId==29020 :
      if player.getActiveWeaponItem() and player.getActiveWeaponItem().getItemId() == PIPETTE_KNIFE and st.getRandom(100)<chance and st.getQuestItemsCount(dropId) == 0:
        ...
        npc.broadcastPacket(CreatureSay(objId,0,"Baium","Who dares to try and steal my noble blood?"))
        st.playSound("Itemsound.quest_itemget")
    return
  ...
  def onKill(self,npc,player,isPet):
    ...
    if npcId in CHEST_SPAWNS.keys() :
      self.addSpawn(CHEST_SPAWNS[npcId], npc.getX(), npc.getY(), npc.getZ(),npc.getHeading(), True, 120000)
    else :
      st = player.getQuestState(qn)
      if not st : return 
      if st.getState() != STARTED : return
